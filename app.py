...
# --- Data Loading and Saving (SQLite) ---
...
def load_data():
  ...
  for row in cursor.execute("SELECT id, name FROM categories"):
      categories.append(row[1])  # Only append the category name

  # Load Session Log.  Join with categories to get category name.
  ...
  '''):
    ...
    # Load total_times
  for row in cursor.execute("SELECT category_id, total_seconds FROM total_times"):
        category_id = row[0]
...
def add_category_to_db(category_name):
    if category_name:
        ...
        try:
            ...
            #Initialize the total time spent to zero
            cursor.execute("INSERT INTO total_times (category_id, total_seconds) VALUES(?,?)", (category_id,0))
            conn.commit()
...
def log_session_to_db(session_data):
    ...
    try:
        ...
        cursor.execute('''
            INSERT INTO session_log (date, start_time, end_time, duration, category_id, work_done, efficiency)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (session_data['date'], session_data['start_time'], session_data['end_time'], session_data['duration'],
              category_id, session_data['work_done'], session_data['efficiency']))
        ...
        cursor.execute('''
            ...
            WHERE category_id = ?
        ''', (session_data['duration_seconds'], category_id))
        conn.commit()
...
def add_category(category_name):
    if category_name and category_name not in st.session_state.categories:
        ...
        st.session_state.categories = load_data()["categories"]
        st.session_state.total_times = load_data()["total_times"]
...
def log_session():
    if st.session_state.current_category:
        ...
        st.session_state.session_log = load_data()["session_log"]  # Refresh log
        st.session_state.total_times = load_data()["total_times"]
        reset_timer()
